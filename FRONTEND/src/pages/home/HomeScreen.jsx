...
const HomeScreen = () => {
  ...
  return (
    ...
    <div className='relative h-screen text-white'>
      ...
      <img src={ORIGINAL_IMG_BASE_URL+ trendingContent?.backdrop_path} alt="hero img" className="absolute top-0 left-0 h-full w-full object-cover -z-50" onLoad={() => {
						setImgLoading(false);
					}}></img>
      <div className='absolute top-0 left-0 h-full w-full bg-black/50 -z-50 ' aria-hidden='true' ></div>
      <div className='absolute top-0 left-0 h-full w-full flex flex-col justify-center px-8 md:px-16 lg:px-32'>
      <div className='bg-gradient-to-b from-black via-transparent to-transparent absolute w-full h-full top-0 left-0 -z-50'></div>
      
        <div className='max-w-2xl'>
        <h1 className='mt-4 text-6xl font-extrabold text-balance'>{trendingContent?.title || trendingContent?.name}</h1>
        <p className='text-lg mt-2'>{trendingContent?.release_date?.split("-")[0] ||
								trendingContent?.first_air_date.split("-")[0]}{" "}
							| {trendingContent?.adult ? "18+" : "PG-13"}</p>
        <p className='text-lg mt-2'> {trendingContent?.overview.length > 200
      ...
      <div className='flex mt-8'>
        ...
        <Play className='size-6  mr-2 fill-black'></Play>Play</Link>
        <Link
